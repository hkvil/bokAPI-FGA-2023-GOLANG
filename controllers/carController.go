...
func DeleteCar(ctx *gin.Context){
	...
	ctx.JSON(http.StatusOK,gin.H{
		"message":fmt.Sprintf("car with id %v has been succesfuly deleted",carID),
	})
}
...
func UpdateCar(ctx *gin.Context){
	...
	if !condition{
		ctx.AbortWithStatusJSON(http.StatusNotFound,gin.H{
			"error_status":"Data Not Found",
			"error_message":fmt.Sprint("car with id %v not found",carID),
		})
	}
	ctx.JSON(http.StatusOK,gin.H{
		"message":fmt.Sprint("car with id %v has been succesfully updated"),
	})
}